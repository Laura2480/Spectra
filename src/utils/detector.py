...
class DetectionLoader():
    def __init__(self, input_source, detector, cfg, opt, mode='image', batchSize=1, queueSize=128):
        ...
        """
        image_queue: the buffer storing pre-2D images for object detection
        det_queue: the buffer storing human detection results
        pose_queue: the buffer storing post-2D cropped human image for pose estimation
        """
        if opt.sp:
            self._stopped = False
        ...
        for i in range(self.num_batches):
            ...
            for k in range(i * self.batchSize, min((i + 1) * self.batchSize, self.datalen)):
                ...
                if not grabbed or self.stopped:
                    # put the rest pre-2D data to the queue
                    if len(imgs) > 0:
                        with torch.no_grad():
