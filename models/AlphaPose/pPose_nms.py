...
def write_json(all_results, outputpath,outputname='alphapose-results.json', for_eval=False):
    '''
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            kp_preds_norm = human['keypoints_normalized']
            ...
            for n in range(kp_scores.shape[0]):
                ...
                keypoints_norm.append(float(kp_preds_norm[n, 0]))
                keypoints_norm.append(float(kp_preds_norm[n, 1]))
                keypoints_norm.append(float(kp_scores[n]))
            result['keypoints'] = keypoints
            result['keypoints_normalized'] = keypoints_norm
    ...
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath,outputname), 'w') as json_file:
