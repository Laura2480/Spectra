...
def motion2video_mesh(motion, save_path, fps=25, keep_imgs = False, draw_face=True):
    ...
    for f in tqdm(range(vlen)):
        ...
        #plt.savefig("/content/drive/MyDrive/Unisa/Spectra/Esperimenti/processed_mesh/res_img/"+str(f)+".png", transparent=True, bbox_inches="tight", pad_inches=0)
        ...
        if draw_skele:
            ...
            ax.plot_trisurf(j3d[:, 0], -j3d[:, 2], triangles=smpl_faces, Z=-j3d[:, 1], color=(166/255.0,188/255.0,218/255.0,0.9))
        else:
